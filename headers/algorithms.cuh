	...
	__host__ __device__ float operator()(int id1, int id2,
		...
		for (int i = 0; i < size; ++i) {
			...
			if (neigbours[base1 + i].w != id2)
				E0 += kernel(n1, sites, neigbours, size, neigbours[base1 + i].w,
						parameters, atomsN);
			if (neigbours[base2 + i].w != id1)
				E0 += kernel(n2, sites, neigbours, size, neigbours[base2 + i].w,
						parameters, atomsN);
		}
		...
		for (int i = 0; i < size; ++i) {
			...
			n2 = static_cast<int>(sites[neigbours[base2 + i].w].w);
			if (neigbours[base1 + i].w != id2)
				E1 += kernel(n1, sites, neigbours, size, neigbours[base1 + i].w,
						parameters, atomsN, exchanger);
			if (neigbours[base2 + i].w != id1)
				E1 += kernel(n2, sites, neigbours, size, neigbours[base2 + i].w,
						parameters, atomsN, exchanger);
		}
...
struct ConstantSaddleEnergy {
	 ...
	 * of interface.
	 */
	__host__ __device__  float operator()(int id1, int id2 , const float4* const sites ,
			const int4 * const neigbours, const float* params ) {

...
struct ReturnOneIfTheSameAtom{
private:
    const IsAtomPredicate _predicate;
public:
    ReturnOneIfTheSameAtom(definitions::Atom atom):_predicate(atom) {

    }

    __host__ __device__ int operator()(const float4& atom1, const float4 atom2) const {
        return _predicate(atom1) && static_cast<uint>(atom1.w) == static_cast<uint>(atom2.w) ? 1: 0;
    }
...
struct FloatExponentWithFactor {
    ...
    FloatExponentWithFactor(float factor):_factor(factor){}

    __host__ __device__ float operator()(float x) {
        return expf(_factor * x);
    }
